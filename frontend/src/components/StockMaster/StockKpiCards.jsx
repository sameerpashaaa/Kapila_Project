...
export function StockKpiCards({ data = {}, filters = {}, handleStatCardClick = () => {} }) {
  ...
  const kpis = [
    { id: "total", label: "Total Active Spend", value: data.total_spend ?? data.totalActiveSpend ?? 0, color: COLORS.teal, icon: <Banknote size={20} /> },
    { id: "active", label: "Current Store Value", value: data.store_value ?? data.currentStoreValue ?? 0, color: COLORS.accent, icon: <PackageOpen size={20} /> },
    { id: "low", label: "Low Stock Value", value: data.low_stock_value ?? data.lowStockValue ?? 0, color: COLORS.danger, icon: <AlertTriangle size={20} /> }
  ];
        ...
        return (
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 24, fontWeight: 700, color: COLORS.text, lineHeight: 1.1 }}>
                ₹{parseFloat(kpi.value || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
            </div>
          </Card>
        );
